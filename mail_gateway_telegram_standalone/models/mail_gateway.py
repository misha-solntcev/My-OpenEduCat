...
class MailGateway(models.Model):
    ...
    def send_message(self, chat_id, message, parse_mode="HTML"):
        ...
        # Whitelist check: if authorized chats are configured,
        # only allow sending to known chat_ids
        if self.telegram_chat_ids:
            if not self.telegram_chat_ids.filtered(
                lambda c, cid=chat_id: c.chat_id == str(cid)
            ):
                _logger.warning(
                    "Telegram bot %s: chat_id %s is not in authorized list, skipping",
                    self.name,
                    chat_id,
                )
                return False
    ...
    def _is_webhook_active(self):
        """Check via Telegram API if a webhook is currently set"""
        self.ensure_one()
        try:
