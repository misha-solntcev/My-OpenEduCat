...
class OpSubjectGrades(models.Model):
    ...
    def _generate_quarter_summary_html(self, stats, final_grade, avg_mark):
        """ 
        Версия 31.0: Исправлено позиционирование бейджика (без CSS translate).
        """
        counts = stats['counts']
        ...
        # Ограничиваем значения для шкалы 2.0 - 5.0
        val_clamped = max(2, min(5, avg_val))
        
        # Координаты X и Y
        avg_x = bar_start_x + ((val_clamped - 2) / 3) * max_bar_w
        # Y: от 84 (оценка 2) до 18 (оценка 5)
        avg_y = 84 - ((val_clamped - 2) / 3) * 66
        ...
        <svg viewBox="0 0 140 100" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" style="width:100%; height:100px;">
            <style>
                ...
                .glass-badge {{ animation: fadeIn 0.5s ease-out forwards; animation-delay: 0.8s; opacity: 0; }}
            </style>
        """
        ...
        for i, g in enumerate([5, 4, 3, 2]):
            ...
            if val > 0:
                svg += f'<rect class="bar" x="{bar_start_x}" y="{y}" width="{bar_w}" height="12" fill="{color}" rx="3" />'

        # --- СЛОЙ 2: ИНДИКАТОР И БЕЙДЖ ---
        if avg_val >= 2:
            # Линия
