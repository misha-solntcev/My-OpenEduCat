...
class MailGatewayTelegramService(models.AbstractModel):
    ...
    def _run_async(self, coro):
        ...
        Falls back to a new thread if an event loop is already running
        in the current thread (prevents RuntimeError from asyncio.run)."""
        try:
    ...
    def _remove_webhook(self, gateway):
        self._run_async(self._remove_webhook_telegram(gateway))
    ...
    def _receive_update(self, gateway, update):
        # Ensure is_bot field is present for compatibility with python-telegram-bot >= 20
        if "message" in update and "from" in update["message"]:
            update["message"]["from"].setdefault("is_bot", False)
    ...
    async def _process_telegram_attachment(self, attachment):
        ...
        if isinstance(attachment, telegram.Sticker):
            ...
            try:
                ...
                _logger.warning("Could not process sticker, sending as-is")
    ...
    def _process_update(self, chat, update):
        ...
        if update.message.effective_attachment:
            ...
            attachment_data = self._run_async(
    ...
    def _update_content_after_hook(self, channel, message):
        self._run_async(self._async_update_content_after_hook(channel, message))
